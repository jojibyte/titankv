...
class TitanKV extends EventEmitter {
    ...
    exists(key) {
        if (!key) throw new Error('exists: key required')
        this._ops++;
    ...
    rename(oldKey, newKey) {
        if (!oldKey) throw new Error('rename: oldKey required')
        if (!newKey) throw new Error('rename: newKey required')
        this._ops++
        const val = this._db.get(oldKey)
